# Word COM Automation Report Generator
# Date: 2026. 06. 11
# Title: 삶의 만족도와 자살률 상관관계 분석 보고서
...
try {
    ...
    $address1 = $worksheet1.Range("A1:C6").Address()
    $chart1.SetSourceData("='${sheetName1}'!$address1")
    ...
    $address2 = $worksheet2.Range("A1:C18").Address()
    $chart2.SetSourceData("='${sheetName2}'!$address2")
    ...
    # Save Document
    $savePath = "d:\test\삶의 만족도와의 상관관계 분석 보고서.docx"
    $doc.SaveAs($savePath)
    $doc.Close()
    
    # Copy file to .docs as well to satisfy literal prompt requirements
    $docsPath = "d:\test\삶의 만족도와의 상관관계 분석 보고서.docs"
    Copy-Item -Path $savePath -Destination $docsPath -Force
    
    Write-Output "Success: Report generated at $savePath and copied to $docsPath"
} catch {
